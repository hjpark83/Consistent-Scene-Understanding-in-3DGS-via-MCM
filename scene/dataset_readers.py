...
def _list_image_files(images_folder):
    ...
    return sorted(
        [
            ...
            if os.path.splitext(name)[1] in image_exts
        ],
        key=lambda path: os.path.basename(path),
    )


def _resolve_colmap_image_path(images_folder, colmap_name, camera_index, image_files):
    basename = os.path.basename(colmap_name)
    exact_path = os.path.join(images_folder, basename)
    if os.path.exists(exact_path):
        return exact_path

    stem = os.path.splitext(basename)[0]
    for image_path in image_files:
        if os.path.splitext(os.path.basename(image_path))[0] == stem:
            return image_path

    lower_basename = basename.lower()
    for image_path in image_files:
        if os.path.basename(image_path).lower() == lower_basename:
            return image_path

    # Some datasets are renamed after COLMAP, e.g. COLMAP keeps IMG_*.jpg
    # while the image folder is normalized to 00.jpg, 01.jpg, ...
    if len(image_files) == len(os.listdir(images_folder)) and camera_index < len(image_files):
        return image_files[camera_index]
    if camera_index < len(image_files):
        return image_files[camera_index]

    raise FileNotFoundError(
        f"Could not find image for COLMAP entry '{colmap_name}' in {images_folder}. "
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, objects_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if intr.model=="SIMPLE_PINHOLE":
            ...
            # Treat COLMAP radial cameras as pinhole for Gaussian training.
            # For best photometric accuracy, pre-undistort the dataset with COLMAP;
            # this fallback keeps LLFF-style SIMPLE_RADIAL reconstructions usable.
            focal_length_x = intr.params[0]
            FovY = focal2fov(focal_length_x, height)
            FovX = focal2fov(focal_length_x, width)
        ...
        image_path = _resolve_colmap_image_path(images_folder, extr.name, idx, image_files)
...
def readColmapSceneInfo(path, images, eval, object_path, llffhold=8, n_views=100, random_init=False, train_split=False):
    ...
    try:
        ...
        # Memory optimization: Downsample point cloud if too large
        max_points = int(os.getenv('MAX_INIT_POINTS', '150000'))
        if len(pcd.points) > max_points:
            ...
            pcd = BasicPointCloud(
                ...
                normals=pcd.normals[indices] if pcd.normals is not None else np.zeros((max_points, 3))
            )
    except:
        pcd = None
